...
This is a significant limitation of JSR-356 that Spring's WebSocket support addresses with a standard `RequestUpgradeStrategy` implementation when running in a WebSocket API 2.1+ runtime.
...
NOTE: For client Jakarta WebSocket configuration, use ContainerProvider.getWebSocketContainer() in programmatic configuration, or `WebSocketContainerFactoryBean` in XML.
...
TIP: When using STOMP over WebSocket, you will also need to configure xref:web/websocket/stomp/server-config.adoc[STOMP WebSocket transport] properties.
 ...
 * Allow all origins: To enable this mode, you should provide `{asterisk}` as the allowed origin value. In this mode, all transports are available.
