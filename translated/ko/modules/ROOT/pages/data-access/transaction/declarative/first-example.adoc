...
The expression defined within the `<aop:pointcut/>` element is an AspectJ pointcut expression. See xref:core/aop.adoc[the AOP section] for more details on pointcut expressions in Spring.
...
NOTE: In the preceding example, it is assumed that all your service interfaces are defined in the `x.y.service` package. See xref:core/aop.adoc[the AOP section] for more details.
