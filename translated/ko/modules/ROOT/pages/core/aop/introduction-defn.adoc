...
* Aspect: A modularization of a concern that cuts across multiple classes. Transaction management is a good example of a crosscutting concern in enterprise Java applications. In Spring AOP, aspects are implemented by using regular classes (the xref:core/aop/schema.adoc[schema-based approach]) or regular classes annotated with the `@Aspect` annotation (the xref:core/aop/ataspectj.adoc[@AspectJ style]).
...
All advice parameters are statically typed so that you work with advice parameters of the appropriate type (for example, the type of the return value from a method execution) rather than `Object` arrays.
